"""
MediPredict AI – Streamlit Frontend (FastAPI-backed)
...
.api-info { background: rgba(0,212,200,0.05); border: 1px solid rgba(0,212,200,0.15); border-radius: 12px; padding: 1rem 1.4rem; margin-bottom: 1rem; font-size: 0.82rem; color: #7b90a8; }
.api-info code { color: #00d4c8; background: rgba(0,212,200,0.1); padding: 2px 6px; border-radius: 4px; font-size: 0.8rem; }
...
with st.sidebar:
    ...
    api_host = st.text_input("FastAPI Host", value="http://localhost:8000")
    st.markdown("---")
    st.markdown("**API Endpoints**")
    st.code("POST /predict/heart\nPOST /predict/diabetes\nPOST /predict/liver\nPOST /predict/lung")
    st.markdown("**Docs:** `{host}/docs`")
...
# ── Helper: call API & show result ───────────────────────────────────────────
def call_api_and_show(endpoint: str, payload: dict, host: str = api_host):
    try:
        with st.spinner("Running prediction via FastAPI..."):
            resp = requests.post(f"{host}{endpoint}", json=payload, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
            pred  = data["prediction"]
            r_pct = data["risk_probability"]
            s_pct = data["safe_probability"]
            label = data["label"]
            conf  = r_pct if pred == 1 else s_pct
            ...
            if pred == 1:
                st.markdown(f"""<div class="result-wrap"><div class="result-positive">
                    ...
                    <div class="prob-row prob-bar-risk"><span class="prob-label">Risk</span>
                        <div class="prob-bar-bg"><div class="prob-bar-fill" style="width:{r_pct:.1f}%"></div></div>
                        <span class="prob-pct">{r_pct:.1f}%</span></div>
                    <div class="prob-row prob-bar-safe"><span class="prob-label">Safe</span>
                        <div class="prob-bar-bg"><div class="prob-bar-fill" style="width:{s_pct:.1f}%"></div></div>
                        <span class="prob-pct">{s_pct:.1f}%</span></div>
            ...
            st.error(f"⚠️ Model not found: {resp.json().get('detail')}")
        elif resp.status_code == 422:
            st.error("⚠️ Validation Error – check input values")
            st.json(resp.json())
        else:
...
st.markdown(f"""
<div class="hero">
  <div class="hero-badge">FastAPI · Pydantic v2 · Streamlit · 4 Diseases</div>
  <div class="hero-title">MediPredict AI</div>
  <div class="hero-sub">Clinical-grade disease risk prediction — Streamlit frontend calling FastAPI backend.</div>
</div>
<div class="acc-grid">
...
<div class="api-info">
  🔌 API Base: <code>{api_host}</code> &nbsp;·&nbsp;
  Interactive docs: <code>{api_host}/docs</code> &nbsp;·&nbsp;
  Configure in sidebar ⚙️
</div>
""", unsafe_allow_html=True)
...
# ── TAB 1: HEART ─────────────────────────────────────────────────────────────
with tab1:
    st.markdown('<div class="section-label">Heart Disease Prediction</div>'
                '<div class="section-sub">POST /predict/heart · 13 clinical indicators · Gradient Boosting</div>',
                unsafe_allow_html=True)
    ...
    with c1:
        ...
        cp       = st.selectbox("Chest Pain Type", [0,1,2,3],
                                format_func=lambda x: {0:"Typical Angina",1:"Atypical Angina",2:"Non-anginal",3:"Asymptomatic"}[x], key="h_cp")
    ...
    with c2:
        ...
        restecg  = st.selectbox("Resting ECG", [0,1,2],
                                format_func=lambda x: {0:"Normal",1:"ST-T Abnormality",2:"LV Hypertrophy"}[x], key="h_ecg")
    ...
    with c3:
        ...
        slope    = st.selectbox("ST Slope", [0,1,2],
                                format_func=lambda x: {0:"Upsloping",1:"Flat",2:"Downsloping"}[x], key="h_slope")
        ca       = st.selectbox("Major Vessels (0–3)", [0,1,2,3], key="h_ca")
        thal     = st.selectbox("Thalassemia", [1,2,3],
                                format_func=lambda x: {1:"Normal",2:"Fixed Defect",3:"Reversible Defect"}[x], key="h_thal2")
    ...
    if st.button("🔍  Run Heart Disease Prediction", key="heart_btn"):
        ...
        call_api_and_show("/predict/heart", payload)

# ── TAB 2: DIABETES ──────────────────────────────────────────────────────────
with tab2:
    st.markdown('<div class="section-label">Diabetes Prediction</div>'
                '<div class="section-sub">POST /predict/diabetes · 8 clinical markers · Gradient Boosting</div>',
                unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    if st.button("🔍  Run Diabetes Prediction", key="diab_btn"):
        payload = {"Pregnancies":preg,"Glucose":gluc,"BloodPressure":bp,"SkinThickness":skin,
                   "Insulin":ins,"BMI":bmi,"DiabetesPedigreeFunction":dpf,"Age":age_d}
        call_api_and_show("/predict/diabetes", payload)

# ── TAB 3: LIVER ─────────────────────────────────────────────────────────────
with tab3:
    st.markdown('<div class="section-label">Liver Disease Prediction</div>'
                '<div class="section-sub">POST /predict/liver · 10 blood test features · Gradient Boosting</div>',
                unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    if st.button("🔍  Run Liver Disease Prediction", key="liver_btn"):
        payload = {"Age":l_age,"Gender":l_gen,"Total_Bilirubin":l_tb,"Direct_Bilirubin":l_db,
        ...
        call_api_and_show("/predict/liver", payload)

# ── TAB 4: LUNG ──────────────────────────────────────────────────────────────
with tab4:
    st.markdown('<div class="section-label">Lung Cancer Risk Prediction</div>'
                '<div class="section-sub">POST /predict/lung · 15 symptom features · SMOTE balanced · Gradient Boosting</div>',
                unsafe_allow_html=True)
    def yn(label, key):
        return st.selectbox(label, [0,1], format_func=lambda x: "No" if x==0 else "Yes", key=key)
    c1, c2, c3 = st.columns(3)
    with c1:
    ...
    if st.button("🔍  Run Lung Cancer Prediction", key="lung_btn"):
        payload = {"GENDER":lu_gen,"AGE":lu_age,"SMOKING":sm,"YELLOW_FINGERS":yf,
        ...
        call_api_and_show("/predict/lung", payload)

st.markdown('<div class="disclaimer">⚠️ For educational purposes only · Not a substitute for professional medical diagnosis</div>',
            unsafe_allow_html=True)
